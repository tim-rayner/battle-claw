...
const defaultPlatform = process.env.PUBG_PLATFORM ?? (rcConfig.platform as string) ?? 'steam';
...
program
  ...
  .option('-p, --players <names>', 'Comma-separated player names (max 4)', defaultPlayers)
  .option('-m, --matches <count>', 'Number of recent matches to analyze', '5')
  .option('--mode <mode>', 'Filter by game mode: solo, duo, squad')
  .option('--platform <shard>', 'Platform shard: steam, kakao, psn, xbox', defaultPlatform)
  .option('-o, --output <format>', 'Output format: table, json, context', 'table')
  ...
  .action(async (opts) => {
    if (!opts.players) {
      console.error('Error: --players is required (or set defaultPlayers in .pubgrc.json)');
      process.exit(1);
    }
...
program
  ...
  .option('--match <matchId>', 'Match ID to analyze (defaults to most recent match for first player)')
  .option('-p, --players <names>', 'Comma-separated player names to focus on', defaultPlayers)
  .option('--focus <aspect>', 'Focus area: aim, weapons, tactics, combat, all', 'all')
...
program
  ...
  .option('-p, --players <names>', 'Comma-separated player names', defaultPlayers)
...
program
  ...
  .action(async (opts) => {
    if (!opts.player) {
      console.error('Error: --player is required (or set defaultPlayers in .pubgrc.json)');
      process.exit(1);
    }
    const api = createApiClient();
